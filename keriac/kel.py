...
class KEL:
    ...
    def __iter__(self) -> Iterator[Event]:
        ...
        for raw in self._identity.habitat.db.getEvtLastPreIter(self._identity.habitat.pre):
            yield Event(raw)

    def __len__(self) -> int:
        """Return the number of events in the KEL."""
        # The sequence number is 0-indexed, so we add 1 to the highest SN.
        return self._identity.habitat.kever.sn + 1

    def __repr__(self):
        return f"KEL(identity='{self._identity._hab.name}', length={len(self)})"
